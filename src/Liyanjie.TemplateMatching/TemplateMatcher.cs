...
namespace Liyanjie.TemplateMatching
{
    public class TemplateMatcher
    {
        ...
        readonly object[] _defaultValues;

        public TemplateMatcher(RouteTemplate template, RouteValueDictionary defaults)
        {
            Template = template ?? throw new ArgumentNullException(nameof(template));
            Defaults = new ReadOnlyDictionary<string, object>(defaults ?? new RouteValueDictionary());
        ...
        public ReadOnlyDictionary<string, object> Defaults { get; }

        public RouteTemplate Template { get; }

        public bool TryMatch(string path, RouteValueDictionary values)
        {
            if (values == null)
            {
            ...
            RouteValueDictionary values)
        {
            ...
            if (routeSegment.Parts[indexOfLastSegment].IsOptional &&
                routeSegment.Parts[indexOfLastSegment - 1].IsOptionalSeparator)
            {
                ...
                {
                    if (requestSegment.EndsWith(routeSegment.Parts[indexOfLastSegment - 1].Text, StringComparison.OrdinalIgnoreCase))
                    {
                        return false;
                    }
